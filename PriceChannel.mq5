  ...
  double avg_close_inperiod(int start,int end){
   ...
   int trenddetect(int start,int end){
      ...
      for (int i =start; i< end; i++){
       ...
       //----Classification---//  
       if(posvalues>(end/2 + 20)) {if (posvalues>(end/2 + 75)) trend = 2; else trend =1;}
       else if(negvalues>(end/2 + 20)) {if (negvalues>(end/2 + 75)) trend = -2; else trend =-1;}
       else trend = 0;
    return trend;
      }   
...
void OnTick()
  {
   ...
   switch(trenddetect(0,Max_candles)){
         case -2:
            HighestF= ArrayMaximum(Closes,Max_candles/2,Max_candles);
            HighestS= ArrayMaximum(Closes,0,(HighestF-25));
            LowestF = ArrayMinimum(Closes,((HighestS)),(HighestF));
         ...
         case -1:
            HighestF= ArrayMaximum(Closes,Max_candles/2,Max_candles);
            HighestS= ArrayMaximum(Closes,0,(HighestF-25));
            LowestF = ArrayMinimum(Closes,((15)),(HighestF));
            ObjectCreate(0,"PriceChannel",OBJ_CHANNEL,0,Priceinfo[HighestF].time,Priceinfo[HighestF].high,Priceinfo[HighestS].time,Priceinfo[HighestS].high,Priceinfo[LowestF].time,Priceinfo[LowestF].low);
            break;
         ...
         case 0:
            ...
            break;
         case 1:
            HighestF= ArrayMaximum(High,(25),(HighestS-26));
            HighestS= ArrayMaximum(High,(Max_candles/2+10),Max_candles);
            LowestF = ArrayMinimum(Lowe,(21),Max_candles);
            ObjectCreate(0,"PriceChannel",OBJ_CHANNEL,0,Priceinfo[HighestF].time,Priceinfo[HighestF].high,Priceinfo[HighestS].time,Priceinfo[HighestS].high,Priceinfo[LowestF].time,Priceinfo[LowestF].low);
            break;
         case 2:
            LowestS= ArrayMinimum(Closes,(120),Max_candles);
            LowestF= ArrayMinimum(Closes,(25),(LowestS-26));
            HighestF = ArrayMaximum(Closes,(LowestF),LowestS);
