...
def ma(period, symbol,time_interval=mt.TIMEFRAME_D1):
    ...
    if len(pos)<period: period=len(pos)-1
...
def RSI(symbol,period=30,time_interval=mt.TIMEFRAME_D1):
    pos=pd.DataFrame(mt.copy_rates_from(symbol,time_interval,datetime.now(),(period)))
    posit=[0]*period
    negit=[0]*period
    close=pos['close'].pct_change()
    ...
    posit=posit.rolling(14).mean()
    negit=negit.rolling(14).mean().abs()
    rs=posit/negit
    rsi=100-(100/(1+rs))
    rsi=rsi[0]
    if rsi[period-1]>70:
        for i in range(1,4):
            if rsi[period-(i+1)]>rsi[period-i]:
                order='SELL'
            else:
                order='NEUTRAL'
    elif rsi[period-1]<30:
        for i in range(1,4):
            if rsi[period-(i+1)]<rsi[period-i]:
                order='BUY'
            else:
...
def MACD(symbol,period=100,time_interval=mt.TIMEFRAME_D1):
    ...
    #For new symbol be not enough lenght
    if len(pos)<period: period=len(pos)
...
def CCI(symbol,period=100,time_interval=mt.TIMEFRAME_D1):
    ...
    if CCI[period-1]<100:
        ...
        ccorder = "NEUTRAL"
    return CCI[period-1],ccorder
##-------------Trend Following-------
def Trend(symbol,time_interval,period=200):
    pos=pd.DataFrame(mt.copy_rates_from(symbol,time_interval,datetime.now(),(period)))
    if len(pos)<period: period=len(pos)
    close=pos['close']
    ema=close.rolling(int(period/21)).mean()
    up=0
    down=0
    for p in range(1,int(period/2)):
        if close[period-p]>ema[period-p]:
            up+=1
        elif close[period-p]<ema[period-p]:
            down+=1
    if up>int(period/4+10):
        trend='UP'
    elif down>int(period/4+10):
        trend='DOWN'
    else:
        trend='NEUTRAL'
...
#Indicators signal
    ...
    for timer in timess:
        for symbol in symbols:
            ...
            parite,MA,masignal,orders,indisignal,trend,timec=technicalsig(symbol,timer)
            imlec.execute('''INSERT INTO dbo.technical_indicators VALUES('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')'''.format(counterr,parite,MA[0],MA[1],MA[2],MA[3],MA[4],masignal,
                                                                                                                                                                    orders[0], orders[1],orders[2],orders[3],orders[4],indisignal,trend,timec,date))
            imlec.commit()
            counterr+=1
...
#Indicators Values        
    ...
    for timer in timess:
        for symbol in symbols:
            ...
            date = datetime.now()
            date = date.strftime('%Y-%m-%d %H:%M:%S')
            parite,MA,orders,pivot,timec=technicalvalue(symbol,timer)
            imlec.execute('''INSERT INTO dbo.technical_indicators_values VALUES({},'{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')'''.format(counter,parite,MA[0],MA[1],MA[2],MA[3],MA[4],
                                                                                                                                                                    orders[0], orders[1],orders[2],orders[3],orders[4],float(pivot[4]),float(pivot[3]),float(pivot[0]),float(pivot[1]),float(pivot[2]),timec,date))
            imlec.commit()
            counter+=1
    First=0
Second=1
...
#Indicators signal
    ...
    for timer in timess:
        for symbol in symbols:
            ...
            date = date.strftime('%Y-%m-%d %H:%M:%S')
            parite,MA,masignal,orders,indisignal,trend,timec=technicalsig(symbol,timer)
            imlec.execute('''UPDATE dbo.technical_indicators SET ma10='{}',ma20='{}',ma50='{}',ma100='{}',ma200='{}',ma_signal='{}',rsi='{}',stoch='{}',macd='{}',alligator='{}',cci='{}',indicator_signal='{}',trend='{}',updated_at='{}' WHERE id={} '''.format(MA[0],MA[1],MA[2],MA[3],MA[4],masignal,
                                                                                                                                                                                                                                            orders[0], orders[1],orders[2],orders[3],orders[4],indisignal,trend,date,countersig))
            imlec.commit()
            countersig+=1
#Indicators Values        
    counterval=1
    for timer in timess:
        for symbol in symbols:
            ...
            imlec.execute('''UPDATE dbo.technical_indicators_values SET ma10='{}',ma20='{}',ma50='{}',ma100='{}',ma200='{}',rsi='{}',stoch='{}',macd='{}',alligator='{}',cci='{}',s2='{}',s1='{}',[pivot]='{}',r1='{}',r2='{}',updated_at='{}' WHERE id={} '''.format(MA[0],MA[1],MA[2],MA[3],MA[4],
                                                                                                                                                                                                                                            orders[0], orders[1],orders[2],orders[3],orders[4],float(pivot[4]),float(pivot[3]),float(pivot[0]),float(pivot[1]),float(pivot[2]),date,counterval))
            imlec.commit()
            counterval+=1
    time.sleep(450)
