...
def convertToBin(filename):
    ...
    with open("C:/Users/Admin/Documents/Fx/candle/{}.png".format(filename),'rb') as file:
            ...
            if len(alldays)<345:
                    ...
                    # create columns for each pattern
                        ...
                        for candle in candle_names:    
                            if candle=='CDLMARUBOZU': 
                                candlename="MARUBOZU"
                                figname="MARUBOZU"
                            elif candle=='CDLDOJI':
                                candlename= "DOJI"
                                figname="DOJI"
                            elif candle=='CDLHANGINGMAN':
                                candlename= "HANGINGMAN"
                                figname="ASILIADAM"
                            else:
                                candlename= 'HAMMER'
                                figname="ÇEKİÇ"
                            ...
                            if candlelist and pos[candle][count]!=0:
                                fig.add_annotation(x=pos.time[count],y=(pos.high.max()*1.015), text= figname,showarrow=False,arrowsize=1,arrowwidth=3, arrowcolor="blue",font=dict(size=15))
                                ...
                                candlelist.append(candlename)
                            if not candlelist and pos[candle][count]!=0:
                            ...
                            #For CHart Name
                                ...
                                fig.update_layout(xaxis_rangeslider_visible=False,plot_bgcolor="white")
                                fig.update_xaxes(rangebreaks=[dict(values=missing)])
                            
                                #Add pointer for candle
                                fig.add_annotation(x=pos.time[count],y=((pos.high.max()*1.005)), text= figname,showarrow=False,arrowsize=1,arrowwidth=3, arrowcolor="blue",font=dict(size=15))
                                ...
                                candlelist.append(candlename)
                            if candlelist and count == (period -2) and candlename=='HAMMER':
                                for x in candlelist:
                                    try:
                                        print(x, symbolname)

                                        fig.write_image("C:/Users/Admin/Documents/Fx/candle/{}{}.png".format(x,symbolname))
                                        imlec.execute('''INSERT INTO dbo.Mumlar VALUES ('{}','{}','{}',CONVERT(varbinary(max),'{}',2))'''.format(x,symbolname,timeframe,convertToBin(x+symbolname)))
                                        imlec.commit()
                                    except:
                                        pass
            counter+=1    
                        ...
                        for candle in candle_names:    
                            ...
                            if candlelist and count == (period -2) and candlename=='HAMMER':
                                for x in candlelist:
                                    ...
                                    fig.write_image("C:/Users/Admin/Documents/Fx/candle/{}{}.png".format(x,symbolname))
                                    imlec.execute('''UPDATE dbo.Mumlar SET image = CONVERT(varbinary(max),'{}',2) WHERE symbol = '{}' AND candle_name='{}' '''.format(convertToBin(x+symbolname),symbolname,x))
                                    imlec.commit()
